...
class DemandExpander(nn.Module):
    ...
    def __init__(self, time_step, ltsp, normalize = True,
                ...
                eps_instock_dph = 1e-3, eps_total_dpph = 1e-3, **kwargs):

        super(DemandExpander, self).__init__(**kwargs)
        if not eps_total_dph > 0:
            raise Value Error("eps_total_dph can't be 0")
        ...
        self.demand_mask = mask_func(time_step, ltsp, min_instock_ratio=min_instock_ratio,
                                     ...
                                     eps_titall_dph = eps_total_dph)

    def forward(self, F, demand, total_dph, instock_dph):
        ltsp_demand = _apply_ltsp_kernel(F, demand, self.ltsp_idx, self.ltsp_kernel)

        ltsp_idph = _apply_ltsp_kernel(F, instock_dph, self.ltsp_idx, self.ltsp_kernel)
        ltsp_dph = _apply_ltsp_kernel(F, total_dph, self.ltsp_idx, self.ltsp_kernel)

        masked_demand = self.demand_mask(ltsp_demand, ltsp_dph, ltsp_idh)
        masked_demand_sp1 = masked_dmand[:, :, self.pos_sp1]
        masked_demand_spN = masked_dmand[:, :, self.pos_spN]
